...
Subroutine FdThreeBodyDecay(n_in,alphaH,MAh,MAh2,MFd,MFd2,MFe,MFe2,MFu,               & 
...
& vPhi,gThh,gTVWp,gTVZ,gTVZp,gFdFdcFdFd,gFdFdcFeFe,gFdFdcFuFu,gFdFdFvFv,gFdFuFvFe,       & 
...
Real(dp),Intent(inout) :: gFdFdcFdFd(6,6,6,6),gFdFdcFeFe(6,6,6,6),gFdFdcFuFu(6,6,6,6),gFdFdFvFv(6,6,9,9),       & 
& gFdFuFvFe(6,6,9,6)

Real(dp),Intent(in) :: gThh(2),gTVWp,gTVZ,gTVZp

Real(dp) :: gFdFdcFdFdi(6,6,6),gFdFdcFeFei(6,6,6),gFdFdcFuFui(6,6,6),gFdFdFvFvi(6,9,9),           & 
& gFdFuFvFei(6,9,6)
...
Call CouplingsFor_Fd_decays_3B(MFd(i_run),i_run,alphaH,MAh,MAh2,MFd,MFd2,             & 
...
gFdFdcFuFui = 0._dp 
Call FdToFdcFuFu(i_run,MFd,MFu,MVZ,MVZp,MVWp,Mhh,cplcFdFdhhL,cplcFdFdhhR,             & 
...
& gTVZtemp,gTVZptemp,gTVWptemp,gThhtemp,deltaM,epsI,check,gFdFdcFuFui)
...
coup2(3) = Conjg(cplcFdFdVZpL(gt1,iIN)) 
coup2(4) = Conjg(cplcFdFdVZpR(gt1,iIN))  
...
coup2(1) = cplcFdFdVZL(iIN,gt3) 
coup2(2) = cplcFdFdVZR(iIN,gt3) 
coup2(3) = Conjg(cplcFdFdVZpL(gt3,iIN)) 
coup2(4) = Conjg(cplcFdFdVZpR(gt3,iIN))  
coup2(5) = cplcFdFdVZL(gt2,gt1) 
coup2(6) = cplcFdFdVZR(gt2,gt1) 
coup2(7) = Conjg(cplcFdFdVZpL(gt1,gt2)) 
coup2(8) = Conjg(cplcFdFdVZpR(gt1,gt2)) 
Call IntegrateGaugeSS(Boson4, mass, coup2, deltaM, epsI,IntegralVVss,NVVss, resR, check) 
If (resR.ne.resR) resR = 0._dp
resC = 2._dp*resR 
If (gt1.Eq.gt3) Then 
...
Real(dp),Intent(in) :: MFd(6),MFe(6),MVZ,MVZp,Mhh(2)
...
Subroutine FdToFdcFuFu(iIN,MFd,MFu,MVZ,MVZp,MVWp,Mhh,cplcFdFdhhL,cplcFdFdhhR,         & 
& cplcFdFdVZL,cplcFdFdVZR,cplcFdFdVZpL,cplcFdFdVZpR,cplcFdFucVWpL,cplcFdFucVWpR,         & 
& cplcFuFdVWpL,cplcFuFdVWpR,cplcFuFuhhL,cplcFuFuhhR,cplcFuFuVZL,cplcFuFuVZR,             & 
& cplcFuFuVZpL,cplcFuFuVZpR,IntegralSs,IntegralSSss,IntegralVs,IntegralVSss,             & 
& IntegralVSst,IntegralVVss,IntegralVVst,NSs,NSSss,NVs,NVSss,NVSst,NVVss,NVVst,          & 
...
Real(dp),Intent(in) :: MFd(6),MFu(6),MVZ,MVZp,MVWp,Mhh(2)
...
If (resS.ne.resS) Then 
Write(*,*) "NaN appearing in the following diagrams: " 
Write(*,*) "Fd->Fd cFu Fu Propagator: VZp,conj[VWp]" 
Write(*,*)  "M_in: ",m_in 
Write(*,*)  "mass: ",mass 
Write(*,*)  "coup: ",coup 
gSum(gt1,gt2,gt3,Isum)= 0._dp  
...
Real(dp),Intent(in) :: MFd(6),MFv(9),MVZ,MVZp,Mhh(2)

Complex(dp),Intent(in) :: cplcFdFdhhL(6,6,2),cplcFdFdhhR(6,6,2),cplcFdFdVZL(6,6),cplcFdFdVZR(6,6),              & 
& cplcFdFdVZpL(6,6),cplcFdFdVZpR(6,6),cplFvFvhhL(9,9,2),cplFvFvhhR(9,9,2),               & 
...
Real(dp),Intent(in) :: MFu(6),MFv(9),MFe(6),MVWp,MFd(6)

Complex(dp),Intent(in) :: cplcFdFucVWpL(6,6),cplcFdFucVWpR(6,6),cplFvFeVWpL(9,6),cplFvFeVWpR(9,6)
...
Isum = 1 
Allocate( gSum(6,9,6, Isum) ) 
Allocate( Contribution(6,9,6, Isum) ) 
gSum = 0._dp  
Contribution = ' ' 
